...
QRCode.createData = function(dataList) {
  ...
  while (true) {
    ...
    QRBitBuffer2set(buffer2, QRCode.PAD0, 8);
    if (buffer2.length >= totalDataCount * 8) {
      break;
    }
    QRBitBuffer2set(buffer2, QRCode.PAD1, 8);
  }
  return QRCode.createBytes(buffer2, rsBlocks);
}

QRCode.createBytes = function(buffer2, rsBlocks) {
  var offset = 0;
  var maxDcCount = 0;
  var maxEcCount = 0;
