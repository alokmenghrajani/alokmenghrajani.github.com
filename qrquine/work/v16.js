...
// fixed module
fs(moduleCount - 8,8, true) 

var bits = ##BCHTypeNumber##;
for (var i = 0; i < 18; i++) {
  var mod = (( (bits >> i) & 1) == 1);
  fs(Math.floor(i / 3),i % 3 + moduleCount - 8 - 3, mod)
}
for (var i = 0; i < 18; i++) {
  var mod = (( (bits >> i) & 1) == 1);
  fs(i % 3 + moduleCount - 8 - 3,Math.floor(i / 3), mod)
}
