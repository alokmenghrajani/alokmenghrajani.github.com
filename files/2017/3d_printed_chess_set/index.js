...
Triangle.prototype.move = function(dx, dy, dz) {
  var dv = new CSG.Vector(dx, dy, dz);
...
function polygonToTriangles(vertices, expected_normal) {
  if (vertices.length < 3) {
    debugger;
  }
  if (vertices.length == 3) {
    var t = new Triangle(vertices[0].pos, vertices[1].pos, vertices[2].pos);
    if (!t.plane.normal.isSame(expected_normal)) {
      //debugger;
    }
    return [t];
  }
  // split vertices into [0-1-last] and [1-2...last]
  var last = vertices[vertices.length-1];
  var t1 = new Triangle(vertices[0].pos, vertices[1].pos, last.pos);

  // We can't mutate vertices
  var v = [];
  for (var i=1; i<vertices.length; i++) {
    v.push(vertices[i]);
  }
  var t2 = polygonToTriangles(v, expected_normal);
  t2.push(t1);
  return t2;
}
...
Solid.prototype.add = function(p0, p1, p2) {
  this.polygons.push(new Triangle(p0, p1, p2));
}
...
Solid.prototype.move = function(dx, dy, dz) {
  for (var i=0; i<this.polygons.length; i++) {
    this.polygons[i] = this.polygons[i].move(dx, dy, dz);
  }
}
  ...
  if (this.solids.length >= 1) {
    ...
    for (var i=0; i<this.solids.length; i++) {
      for (var j=0; j<this.solids[i].polygons.length; j++) {
        var polygon = this.solids[i].polygons[j];
...
/*    var t = new Triangle(p0, p1, p2);
    if (t.plane.normal.isSame(n)) {
      console.log("a");
    } else {
      t = new Triangle(p0, p2, p1);
      if (t.plane.normal.isSame(n)) {
        console.log("b");
      } else {
        console.log("c");
